...
- 日志记录设计有问题
    - 欠缺信息：日志往往只包含 CUDA 等软件层以及网络与简单的硬件信息输出，缺失与停机关系更大的功耗、核心温度等指标
        - 案例：优先流量控制帧（PFC）未被日志及时记录导致定位困难
    - 冗余信息：环境变量、警告等信息大部分情况下无用
- 人工核查的问题
    - 工程师只会在任务彻底停止后收到通知，对于性能 degradation 无法及时响应
    - 人工核查涉及多个团队之间的协作

![](/uploads/survey-nsdi25/26.png)

这张表展现了各种错误的类型以及在多少的 case 中观察到了对应的异常指标。结论：

- 硬件/GPU 故障频发
- PCIe 总线降速、网卡掉线与网络指标强相关

另外一些深入到秒级别观测指标的 measurement 为我们提供了可以用于观测故障的敏感指标：

- CPU/GPU 负载：故障机利用率会骤降，受影响的机子则会因为 k8s 等的 timeout 过一段时间才停止
- PFC 包速率：网卡缓冲区积压会导致 PFC 包速率飙升，健康机则不会有这一现象
...
- 在不同的工况下，正常指标的定义不一样（高时钟频率下 GPU 温度升高属正常），传统方式会因为缺少新的标签难以训练。解决方式：采用无监督聚类
- 故障与原因之间不是一一对应（一因多果、一果多因）。解决方式：构建指标传播图
- 噪声干扰。解决方式：滑动窗口、时序建模

## Design

![](/uploads/survey-nsdi25/29.png)

工作流程首先从收集指标开始，本文要求日志系统精简后，以秒级别输出。

每个指标都会有针对性设计的 VAE 降噪模型，并且根据指标排序（先前统计了不同的指标对错误指示的概率是不同的）确定需要检测的指标。

![](/uploads/survey-nsdi25/27.png)

确定了需要检测的指标后，对于属于同一组的机器，其各项指标应当接近，所以对每一个指标，统计每一台机子距离其他机子的距离，出现离群点时即说明有可能出现了故障。例如，上图中出现故障的机子 PFC 包速率显著上升。

![](/uploads/survey-nsdi25/28.png)

在确定了可能的故障机子后，对各项指标作监测。由于噪声（传感器抖动、网络等）一般持续若干秒，而故障持续以分钟计时，所以对指标应用一个滑动窗口（例如 5 分钟），只有持续性超过阈值才会认为该指标异常。
...
$i$ 是机器下标，$j$ 是指标下标。同一个指标的 Z-score 离散程度越大说明该指标有问题。

通过长时间的人工标注得到指标优先级排序：

![](/uploads/survey-nsdi25/31.png)
...
![](/uploads/survey-nsdi25/32.png)

对不同的故障类型：

![](/uploads/survey-nsdi25/33.png)
...
第二篇整体就比较模糊，模型的选择、训练以及最后错误原因定位（其实说白了也是一个经验图，就那个表 1）也相对模糊。
