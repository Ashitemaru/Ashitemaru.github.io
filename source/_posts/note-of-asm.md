...
原因是简单的，现代的 CPU 在从主存之中读取数据的时候是同时读取多个，比如说一次性读取 8 个字节。实际上这种读取方式就将每 8 个字节的存储空间划分为了一个“内存块”，如果数据在同一个“内存块”之中，那么一次读取就可以将所有数据读出来。但如果数据跨越了两个“内存块”，那么就需要两次读取。

所以说编译器会故意在存储结构的空间内部塞入若干的空白空间让每一个成员的数据不跨越“内存块”以保证读取效率。
...
这里，结构 `S1` 占用 24 字节，而结构 `S2` 占用 16 字节。这是因为 `S1` 在存放 `a` 后不得不空出 7 字节，否则 `next` 就会跨“内存块”。而 `S2` 中可以先把 `a, b` 都放置好之后，只需要空出 6 字节就可以让 `next` 不跨“内存块”。
