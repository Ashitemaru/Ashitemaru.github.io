...
「である」的另外一种用途是用于补全「です、だ」缺失的部分活用。例如「です、だ」需要接续助词「ば」表示假定的时候，「だ」可以使用「ならば」的形式，但是「です」没有假定形，此时只能使用「である」代替表达为「であれば」。
...
- 友人**である**高橋さんと一緒に参加します。（与朋友高桥先生一起出席）

而这种用法常常会表现两个体言之间的同位关系（即两个体言所指向的事物是同一个），可以使用表同位关系的格助词「の」代替：

- 友人**の**高橋さんと一緒に参加します。（与朋友高桥先生一起出席）
...
事实上，可能式的主语还可以用「に・には」之类的助词标注，但是这类标注一般用于可能式的否定：
...
{% note info no-icon %}
之前提到过实意形容词「ない」和助动词「ない」的区别，理论上这两者在接续「そうだ②」的时候应该分别形成「なさそうだ」和「なそうだ」，是存在对立的。但是近年来这一对立已经基本上消耗完毕，即随着实意形容词和助动词的合流，各种能够区分这两者的语法现象均在消逝：

- （○）小林君は行か**なそうだ**。
- （△）小林君は行か**なさそうだ**。（小林像是不去）
- （×）小林君は楽しく**なそうだ**。
- （○）小林君は楽しく**なさそうだ**。（小林像是不高兴）

事实上，这一差别还有模糊之处，比如本身以「ない」结尾的形容词「危ない、つまらない、申し訳ない」。从严格的语法上讲「危なそうだ、つまらなそうだ、申し訳な**さ**そうだ」是正确的，但是除了「申し訳ない**さ**そうだ」基本是共识用法外，「つまらな**さ**そうだ」有部分人也会使用，而「危な**さ**そうだ」虽然有人用但是仅限极少数。

追根溯源，「申し訳ない」作为合成词的痕迹过于明显，也就是说实意形容词「ない」过于突出，所以共识上使用「申し訳な**さ**そうだ」。而「つまらない」语源是「つまる」的否定，虽然即使从语源的角度也不应该使用「つまらな**さ**そうだ」，但是随着「ない」的区分模糊，这类用法也有不少人开始使用。而「危ない」只是单纯以「ない」这两个假名结尾而已，和「ない」没有一点关系，然而受到使用惯性而说出「危な**さ**そうだ」的人也逐步出现了。

语言本身就是在变化的，也许什么时候就规定所有以「ない」结尾的词接续「そうだ②」都要用「なさそうだ」呢。
{% endnote %}

{% note info no-icon %}
采用「なさそうだ、よさそうだ」这种莫名多一个「さ」的接续方式的主要原因是「ない、よい」词干是单音节的，很容易在使用中多出一个辅助音节。然而对于「濃い」这类不常用的单音节形容词反倒还按照语法规定接续为「濃そうだ」（「濃さそうだ」也是可以用的，但具体何者使用频率高目前无从可知），然而另外一个不常用的单音节形容词「酸い」反倒会使用其同义词「酸っぱい」代替而形成「酸っぱそうだ」这样的表达。
{% endnote %}
